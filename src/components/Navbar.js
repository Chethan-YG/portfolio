...
const Navbar = () => {
  ...
  return (
    <header>
      <nav className={`navbar navbar-expand-md navbar-light bg-white fixed-top shadow ${isOpen ? "navbar-open" : ""}`}>
        <div className="container-fluid ps-0">
          <div className="d-flex align-items-center">
            <div className="navbar-brand ps-3">
              <img src={profile} alt="Profile Pic" width="50" height="50" className="d-inline-block align-text-top rounded-circle" />
            </div>
          ...
          <button className="navbar-toggler border-0" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNavDropdown"
            aria-controls="navbarNavDropdown" aria-expanded={isOpen} aria-label="Toggle navigation"
            onClick={toggleNavbar} onFocus={(e) => e.target.blur()}>
            <span><i className={`bi display-2 ${isOpen ? "bi-x" : "bi-list"}`}></i></span>
          </button>
